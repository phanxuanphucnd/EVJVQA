...
from transformers import CLIPVisionModel, SwinModel, DetrModel, ViTModel
...
class JoinT5Encoder(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        self.vison_model = "facebook/deit-base-patch16-224"
        ...
        if 'clip' in self.vison_model:
            self.encoder_img = CLIPVisionModel.from_pretrained(self.vison_model)
        elif 'detr' in self.vison_model:
            self.encoder_img = DetrModel.from_pretrained(self.vison_model)
        elif 'vit' in self.vison_model:
            self.encoder_img = ViTModel.from_pretrained(self.vison_model)
        elif 'swin' in self.vison_model:
            self.encoder_img = SwinModel.from_pretrained(self.vison_model)
        else:
            self.encoder_img = AutoModel.from_pretrained(self.vison_model)

        self.img_prj = nn.Linear(256, 768)
    ...
    def forward(
        ...
        V_L = image_embeds.size(1)
        # image_embeds = self.img_prj(image_embeds)
        # image_embeds = torch.cat((image_embeds, image_embeds, image_embeds), 2)
